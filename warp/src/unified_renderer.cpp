...
#define WARP_FLUSH_RENDERER(renderer_obj) unified_renderer::flush_renderer_raii_t flush_renderer_raii ## __LINE__ { renderer_obj }
...
void unified_renderer::create_frame_buffer_images(const COMPUTE_IMAGE_TYPE color_format) {
	...
	// also update/set the warp camera setup whenever the frame buffer changes
	cam_setup = {
		.screen_width = frame_dim.x,
		.screen_height = frame_dim.y,
...
void unified_renderer::create_skybox() {
	...
	skybox_tex = warp_state.rctx->create_image(*warp_state.rqueue,
											   skybox_dim,
											   image_type,
...
bool unified_renderer::resize_handler(EVENT_TYPE type, shared_ptr<event_object>) {
	if (type == EVENT_TYPE::WINDOW_RESIZE) {
		...
		for (uint32_t i = 0; i < max_frames_in_flight; ++i) {
			frame_objects[i].rebuild_pipelines = true;
		}
		return true;
	}
	return false;
...
void unified_renderer::create_passes() {
	...
	
	{
		...
		scatter_passes[0] = warp_state.rctx->create_graphics_pass(pass_desc);
		change_load_ops_to_load(pass_desc);
		scatter_passes[1] = warp_state.rctx->create_graphics_pass(pass_desc);
	}
	
	{
		...
		gather_passes[0] = warp_state.rctx->create_graphics_pass(pass_desc);
		change_load_ops_to_load(pass_desc);
		gather_passes[1] = warp_state.rctx->create_graphics_pass(pass_desc);
	}
	
	{
		...
		gather_fwd_passes[0] = warp_state.rctx->create_graphics_pass(pass_desc);
		change_load_ops_to_load(pass_desc);
		gather_fwd_passes[1] = warp_state.rctx->create_graphics_pass(pass_desc);
	}
	
	{
		...
		shadow_pass = warp_state.rctx->create_graphics_pass(pass_desc);
	}
	
	{
		...
		blit_pass = warp_state.rctx->create_graphics_pass(pass_desc);
	}
}
...
bool unified_renderer::create_pipelines() {
	...
	
	{
		...
		if (warp_state.use_tessellation) {
			pipeline_desc.tessellation = {
				.max_factor = warp_state.rdev->max_tessellation_factor,
				.vertex_attributes = {
		...
		scatter_pipeline = warp_state.rctx->create_graphics_pipeline(pipeline_desc);
		if (!scatter_pipeline) {
	...
	}
	{
		...
		gather_pipeline = warp_state.rctx->create_graphics_pipeline(pipeline_desc);
		if (!gather_pipeline) {
	...
	{
		render_pipeline_description pipeline_desc {
			.vertex_shader = shaders[warp_state.use_tessellation ? SCENE_GATHER_FWD_TES : SCENE_GATHER_FWD_VS],
		...
		gather_fwd_pipeline = warp_state.rctx->create_graphics_pipeline(pipeline_desc);
	...
	
	{
		...
		blit_pipeline = warp_state.rctx->create_graphics_pipeline(pipeline_desc);
		if (!blit_pipeline) {
			log_error("failed to create blit_pipeline");
			return false;
		}
		
		pipeline_desc.fragment_shader = shaders[BLIT_DEBUG_DEPTH_FS];
		pipeline_desc.debug_label = "blit_debug_depth_pipeline";
		blit_debug_depth_pipeline = warp_state.rctx->create_graphics_pipeline(pipeline_desc);
		if (!blit_debug_depth_pipeline) {
			log_error("failed to create blit_debug_depth_pipeline");
			return false;
		}
		
		pipeline_desc.fragment_shader = shaders[BLIT_DEBUG_MOTION_2D_FS];
		pipeline_desc.debug_label = "blit_debug_motion_2d_pipeline";
		blit_debug_motion_2d_pipeline = warp_state.rctx->create_graphics_pipeline(pipeline_desc);
		if (!blit_debug_motion_2d_pipeline) {
			log_error("failed to create blit_debug_motion_2d_pipeline");
			return false;
		}
		
		pipeline_desc.fragment_shader = shaders[BLIT_DEBUG_MOTION_3D_FS];
		pipeline_desc.debug_label = "blit_debug_motion_3d_pipeline";
		blit_debug_motion_3d_pipeline = warp_state.rctx->create_graphics_pipeline(pipeline_desc);
		if (!blit_debug_motion_3d_pipeline) {
			log_error("failed to create blit_debug_motion_3d_pipeline");
			return false;
		}
		
		pipeline_desc.fragment_shader = shaders[BLIT_DEBUG_MOTION_DEPTH_FS];
		pipeline_desc.debug_label = "blit_debug_motion_depth_pipeline";
		blit_debug_motion_depth_pipeline = warp_state.rctx->create_graphics_pipeline(pipeline_desc);
		if (!blit_debug_motion_depth_pipeline) {
			log_error("failed to create blit_debug_motion_depth_pipeline");
			return false;
		}
	}
	...
	// create fences
	for (auto& frame : frame_objects) {
		frame.render_post_tess_update_fence = warp_state.rctx->create_fence(*frame.dev_queue);
...
void unified_renderer::encode_indirect_pipelines(const uint32_t frame_idx) {
	...
	if (warp_state.use_tessellation) {
		...
		frame.indirect_scene_scatter_pipeline->add_render_command(*warp_state.rdev, *scatter_pipeline)
			.set_arguments(// tes
		...
		frame.indirect_scene_gather_pipeline->add_render_command(*warp_state.rdev, scene_gather_pipeline)
			.set_arguments(// tes
						   model->materials_arg_buffer,
						   frame.frame_uniforms_buffer,
						   // fs
	...
	frame.indirect_skybox_scatter_pipeline->add_render_command(*warp_state.rdev, *skybox_scatter_pipeline)
		.set_arguments(frame.frame_uniforms_buffer, frame.indirect_sky_fs_data)
		.draw(3u);
	frame.indirect_skybox_gather_pipeline->add_render_command(*warp_state.rdev,
...
void unified_renderer::render() {
	// NOTE: this is called from the outside, potentially triggering the rendering of the next frame
	// NOTE: only one call is active at a time
	
	// early abort if: a) max amount of frames are in flight, b) the renderer is halted, c) warping is currently active in a frame
	if (in_flight_frames >= max_frames_in_flight || halt_renderer || warp_active) {
		return;
	}
	
	try {
		const auto render_frame_idx = next_frame_idx;
		auto& next_frame = frame_objects[render_frame_idx];
		if (!next_frame.present_done) {
			return;
		}
		...
		bool run_warping = false;
		{
			...
			run_warping = (deltaf < frame_limit || warp_state.is_frame_repeat) && !warp_state.is_always_render;
			
			// have we reached the warp limit?
			...
			if (run_warping &&
				...
				warp_frame_num >= in_between_frame_count &&
				!warp_state.is_frame_repeat) {
				// yes -> render again
				run_warping = false;
			}
			
			// if we want to perform warping, but there still is a (render) frame active, abort and try again later,
			// otherwise we'd access the same resources concurrently + ordering wouldn't be right
			if (run_warping && in_flight_frames > 0) {
				return;
			}
			
			// once we decide to perform warping, we only want one frame to be active at a time (see early abort at the start)
			warp_active = run_warping;
			
			// update/init warp state
			next_frame.warp.is_active = run_warping;
			if (run_warping) {
				next_frame.warp.frame_num = warp_frame_num++;
			}
			
			// need to reset when over the limit or we're not warping any more
			if (!run_warping || deltaf >= frame_limit ||
				(warp_state.is_frame_repeat && warp_frame_num >= in_between_frame_count)) {
				render_delta = deltaf;
				time_keeper = now;
				warp_frame_num = 0;
			}
			
			next_frame.render_delta = render_delta;
			next_frame.frame_delta = deltaf;
		}
		
		// start a new frame
		frame_render_state = 0;
		warp_log_wip("-> new frame: $", run_warping ? "warp" : "render");
		++in_flight_frames;
		next_frame.rthread->render();
		if (!run_warping) {
...
void render_thread::run() {
	try {
		...
		const auto is_warping = frame.warp.is_active.load();
		...
		// blit the rendered or the warped frame?
		bool blit_rendered_frame = false;
		// which frame should be used for rendering?
		auto active_frame = &frame;
		if (is_warping) {
			// NOTE: "previous" and "current" frame are "swapped" here, because warping is always one frame (index) ahead
			active_frame = &prev_frame;
			
			if (warp_state.is_warping) {
				warp_log_wip(">> warp ($ | $), fd: $, rd: $", frame_idx, frame.warp.frame_num, frame.frame_delta, frame.render_delta);
				renderer.render_kernels(frame.frame_delta, frame.render_delta, frame.warp.frame_num, prev_frame, frame);
				blit_rendered_frame = false;
				warp_log_wip("<< warp");
			} else {
				warp_log_wip("<<>> warp blit");
				blit_rendered_frame = true;
				renderer.frame_render_state.store(1u);
			}
		} else {
			warp_log_wip(">> render ($), fd: $", frame_idx, frame.frame_delta);
			renderer.render_full_scene(frame_idx, frame.frame_delta);
			warp_log_wip("<< render");
			blit_rendered_frame = warp_state.is_render_full;
			if (!warp_state.is_render_full) {
				// -> previous frame contains warp result
				active_frame = &prev_frame;
			}
		}
		...
		// blit to window
		{
			...
			switch (blit_mode) {
				case 0:
				default:
					blit_pipe = renderer.blit_pipeline.get();
				...
				case 1:
					...
					blit_image = active_frame->scene_fbo.depth.get();
					break;
				case 2:
					blit_pipe = renderer.blit_debug_motion_3d_pipeline.get();
					blit_image = active_frame->scene_fbo.motion[0].get();
					break;
				case 3:
					blit_pipe = renderer.blit_debug_motion_2d_pipeline.get();
					blit_image = active_frame->scene_fbo.motion[0].get();
					break;
				case 4:
					blit_pipe = renderer.blit_debug_motion_2d_pipeline.get();
					blit_image = active_frame->scene_fbo.motion[1].get();
					break;
				case 5:
			...
			present_renderer->draw(blit_draw_info, *blit_image);
			
			present_renderer->end();
			present_renderer->present();
			
			// NOTE: must block this thread now that rendering is fully async, so that we can safely reuse all buffers for the next frame
			present_renderer->commit_and_finish();
			warp_log_wip("$ blit done ($)", is_warping ? "warp" : "render", frame_idx);
...
void unified_renderer::render_kernels(const float& delta, const float& render_delta, const uint32_t& warp_frame_num,
	...
	//
	bool is_first_warp_frame = (warp_frame_num == 0);
	...
	if (warp_state.is_debug_delta) {
		if (dbg_delta >= (1.0f - delta_eps)) {
			dbg_delta = delta_eps;
			is_first_warp_frame = true;
		} else {
			is_first_warp_frame = (dbg_delta == 0.0f);
			dbg_delta += delta_eps;
		}
	...
	if (warp_state.is_scatter) {
		err = libwarp_scatter_floor(&cam_setup, relative_delta,
									is_first_warp_frame && warp_state.is_clear_frame,
	...
	if (err != LIBWARP_SUCCESS) {
		log_error("libwarp error: $", err);
	}
	
	// flag for next render/warp
...
void unified_renderer::flush_renderer_begin() {
	renderer_flush_lock.lock();
	halt_renderer = true;
	for (uint32_t i = 0; i < max_frames_in_flight; ++i) {
...
void unified_renderer::flush_renderer_end() {
	halt_renderer = false;
	renderer_flush_lock.unlock();
}
